...
Pre-configured **Qwen3-30B-A3B-Instruct-2507** (BF16) inference server for the DGX Spark — a plain
...
| Setting | Value |
|---------|-------|
| Model | `Qwen/Qwen3-30B-A3B-Instruct-2507` (Apache-2.0) |
...
| Memory limit | 88Gi (container OOM, never host) — footprint ~82GB (weights + KV + overhead) |
...
- `gpu_memory_utilization` (default `0.70` — ceiling sized to the ~82GB footprint).
